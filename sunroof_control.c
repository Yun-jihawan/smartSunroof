/*
 ...
 즉, 내기, 외기모드만 정해주면 환기 팬은 항상 가동중이고 여기서 에어컨/히터를 사용하겠다 하면 해당 팬에다가 그에 맞는 공기를 주입해주는 것

 환기 팬의 디폴트는 내기 순환모드, Smart_Sunroof_Control 함수를 통해 내기/외기 를 결정하고 이를 main에서 fan을 on 시키면 될 것으로 보임
 그 후 에어컨/히터 사용을 결정하면 될듯

 아래 조건문을 보면 in_out_mode 변수에 값을 안넣은 조건들이 있는데 해당 부분은 디폴트 값을 사용하겠다는 뜻. 즉, 별다른 말이 없으면 내기 모드로 순환환
 */
